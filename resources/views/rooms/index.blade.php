                        ...
                        <div class="block w-full overflow-x-auto">
                          ...
                          <table class="items-center w-full bg-transparent border-collapse">
                            ...
                            @foreach ($rooms as $room)
                                ...
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                  @if ($room->floor_number == 0)
                                      Ground Floor                                     
                                  @endif
                                  @if ($room->floor_number == 1)
                                      First Floor
                                  @endif
                                  @if ($room->floor_number == 2)
                                      Second Floor
                                  @endif
                                </td>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                ...
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 flex justify-center">
                                  <div class="m-2">
                                    <a href="{{ route('rooms.edit', $room) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Edit</a>
                                  </div>
                                  <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                                  onclick="
                                    deleteconfirm({{$room->id}})
                                  ">Delete</button>
                                  <form action="{{ route('rooms.destroy', $room) }}" method="post" id="{{'form-delete-'.$room->id}}">
                                    @csrf
                                    @method('DELETE')
                                  </form>
                                </td>
                          ...
                          @else
                          <p class="px-10 py-5 text-lg text-gray-500 text-center uppercase">There are no Rooms Please add them</p>
                          @endif
                        </div>
                      </div>
                    </div>
